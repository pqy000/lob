...
def preprocess(id):
    ...
    for i in range(num1):
        ...
        for elem in files2:
            ...
            if id in elem:
                ...
                perc = (pm - pts) / pts
                for x in perc:
                    if x > threshold: temp = 4
                    elif x <= threshold and x > threshold2: temp = 3
                    elif x <= threshold2 and x > -threshold2: temp = 2
                    elif x <= -threshold2 and x > -threshold: temp = 1
                    elif x <= -threshold: temp = 0
                    label.append(temp)
    ...
    for i in range(range(dat)):
        worksheet.write(i, 0, str(float(dat[i,0]) / 10000.0))
...
#     workbook = xlwt.Workbook(encoding= 'ascii')
#     worksheet = workbook.add_sheet("My new Sheet")
#     worksheet.write(0, 0,"predicted label")
#     for i in range(1,len(a)+1):
#         worksheet.write(i, 0, a[i-1])
#     workbook.save("new_table.xls")
# fun3_2_2()
